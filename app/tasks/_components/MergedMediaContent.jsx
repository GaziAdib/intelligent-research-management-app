...
//       const session = await auth();
//       const userId = session?.user?.id
//       const mediaContents = await fetchMergedMediaContents(teamId, userId);
  
//       return (
//         <div className="p-4">
//           <h1 className="text-2xl font-bold mb-6 text-gray-800 dark:text-white">Team Media Gallery</h1>
          
//           {mediaContents?.data?.length === 0 ? (
//             <div className="text-center py-10">
//               <p className="text-gray-500 dark:text-gray-400">No media files found for this team</p>
//             </div>
//           ) : (
//             <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
//               {mediaContents?.data?.map((content, index) => (
//                 <div 
//                   key={index} 
//                   className="bg-white dark:bg-gray-800 rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-shadow duration-300"
//                 >
//                   {/* Display media - assuming mediaUrls contains image URLs */}
//                   {content?.mediaUrls?.map((url, index) => (
//                     <div key={index} className="relative aspect-square">
//                     <img
//                       src={url}
//                       alt={`Media content from task ${content?.task?.taskTitle || ''}`} 
//                       className="absolute w-full h-full object-cover"
//                       sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"
